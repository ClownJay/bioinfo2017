...
def paired_kmer_to_connect(kmer):
    connect = defaultdict(list)
    for key in kmer.keys():
        for read2 in kmer[key]:
            prefix = ''.join(key[:-1]) + ' ' + ''.join(read2[:-1])
            suffix = ''.join(key[1:]) + ' ' + ''.join(read2[1:])
...
def paired_path_to_text(path, text_len):
    read1_text, read2_text = map(str, path[0].split(' '))
    for i in range(1, len(path)):
        read1, read2 = map(str, path[i].split(' '))
        read1_text += read1[-1]
        read2_text += read2[-1]
    add_len = 2 * len(read2_text) - text_len
    if read1_text[-add_len-1:] == read2_text[:add_len+1]:
        text = read1_text + read2_text[add_len+1:]
    else:
        text = 'there is no string spelled by the gapped patterns'
    return text

with open('rosalind_ba3l.txt', 'r') as fbj:
    lines = fbj.readlines()
    k, d = map(int, lines[0].strip().split(' '))
    kmer = defaultdict(list)
    # 因为提供了所有的kd kmer所以可以直接算出text的长度
    text_len = len(lines) + 2 * k + d - 1
    for i in range(1, len(lines)):
        read1, read2 = map(str, lines[i].strip().split('|'))
        kmer[read1].append(read2)
    connect = paired_kmer_to_connect(kmer)
    path = eulerian_path(connect)
    text = paired_path_to_text(path, text_len)
    print(text)
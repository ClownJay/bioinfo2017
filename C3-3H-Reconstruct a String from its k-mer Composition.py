...
def kmer_to_connect(kmer_list):
    ...
    for kmer in kmer_list:
        prefix = str(pattern_to_number(list(kmer[:-1])))
        suffix = str(pattern_to_number(list(kmer[1:])))
...
def path_to_text(path, k):
    # 注意存储的node 为前缀后缀，所以需要转换回来的时候k-1
    text = number_to_pattern(int(path[0]), k-1)
    for i in range(1, len(path)):
        text += number_to_pattern(int(path[i]), k-1)[-1]
    return text

with open('rosalind_ba3h.txt', 'r') as fbj:
    lines = fbj.readlines()
    k = int(lines[0].strip())
    kmer_list = []
